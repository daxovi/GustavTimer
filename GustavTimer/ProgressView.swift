        ...
        GeometryReader { geometry in
            HStack(spacing: 5) {
                ForEach(Array(viewModel.timers.enumerated()), id: \.offset) { index, timer in
                    ProgressBarView(progress: (index == viewModel.activeTimerIndex) ? $viewModel.progress : (index < viewModel.activeTimerIndex) ? .constant(1.0) : .constant(0.0), duration: $viewModel.duration)
